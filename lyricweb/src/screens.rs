...
pub fn use_screens() -> ReadSignal<Vec<ScreenDetailed>, LocalStorage> {
    ...
    spawn_local(async move {
        ...
        match get_screens_detailed(&details) {
            Ok(detailed) => {
                write_screens.set(detailed);
            }
            Err(e) => {}
        }
        ...
        _ = use_event_listener(
            ...
            move |_: Event| match get_screens_detailed(&details) {
                Ok(detailed) => {
                    write_screens.set(detailed);
                }
                Err(e) => {}
...
fn get_screens_detailed(details: &ScreenDetails) -> Result<Vec<ScreenDetailed>, JsValue> {
    Ok(details
        .screens()
        .iter()
